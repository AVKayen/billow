...
import { COVERS, PLAYER } from "~/constants";
...
const Player = () => {
    ...
    useEffect(() => {
        if (audioRef.current && currentTrack) {
            ...
            audio.src = `${PLAYER}${currentTrack.album}/${currentTrack.track_number}`;
            audio.load();
            setIsPlaying(true);
    ...
    useEffect(() => {
        if (audioRef.current && currentTrack) {
            audioRef.current.src = `${PLAYER}${currentTrack.album}/${currentTrack.track_number}`;
                ...
                {currentTrack && (
                    <>
                        ...
                        <div className="track-info">
                            <img className="cover" src={`${COVERS}${currentTrack.album}`} alt={currentTrack.title} />
                            <div className="title-artist-bloc">
                                <p className="title">{currentTrack.title}</p>
                                <p className="artist">{currentTrack.artist}</p>
                            </div>
                            <div className="progress-bar">
                                <progress value={currentTime} max={currentTrack.duration} onClick={onProgressBarClick} />
                            </div>
                            <div className='time'>
                                <p className="current"><Timer seconds={currentTime} /></p>
                                <p className="total"><Timer seconds={currentTrack.duration} /></p>
                            </div>
                        </div>
                ...
                <div id="queue-list">
                    {queue.map((item, index) => (
                        <div className="track" key={index}>
                            <img className="cover" src={`${COVERS}${item.album}`} alt={item.title} />
                            <div className="title-artist-bloc">
                                <p className="title">{item.title}</p>
                                <p className="artist">{item.artist}</p>
                            </div>
                            <p className="duration">
                                <Timer seconds={item.duration} />
                            </p>
